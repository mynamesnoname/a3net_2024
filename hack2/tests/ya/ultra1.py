...
from keras.layers import RandomRotation, RandomFlip, RandomTranslation
...
model = Sequential()
model.add(RandomRotation(0.2, fill_mode='nearest', input_shape=input_shape))
model.add(RandomFlip('horizontal_and_vertical'))
model.add(RandomTranslation(width_factor=0.2, height_factor=0.2, fill_mode='nearest'))
...
plt.title('RITFCNN Results (with validation) after {:.0f} epochs, MSE={:.4f}'.format(epochs, MSE))

plt.scatter([], [], c='C0', label='model predictions')
plt.plot([], [], label='one-to-one line', c='C1', ls='--')
plt.legend()
plt.savefig('/data/wbc/A3net/hack2/tests/ya/ultra1.png')
plt.clf()
...
plt.ylabel('loss')
plt.legend()
plt.savefig('/data/wbc/A3net/hack2/tests/ya/ultra1_loss.png')
